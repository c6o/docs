...
```
sample-project-core.sample-project
  Variants:
    UserID: 01H8X7K3TWHRMW0E2RWGSKGV2G
    Ports:
      Local Port: 3000
      Remote Port: 3000
    Condition:
...
:::caution
You will want to exclude the Core service in the consume rules to reduce confusion and local port conflicts. Currently, Codezero maps ports to the `127.72.0.0/14` range but local apps that bind to `0.0.0.0` or `localhost` will encounter port conflicts as they attempt to bind to all local interfaces.
