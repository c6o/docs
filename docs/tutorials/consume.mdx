...
In this tutorial, we learn how to Consume services running in a Kubernetes Cluster. If you recall, the following diagram represents our Application Service Graph:
...
Instead, given that we have a Kubernetes cluster with these services running, we would like to do the following:
...
We say that we would like to be able to consume the Core and Sockets services. Concretely, we would like to be able to run the following `curl` or `websocat` commands and retrieve data from these services as though they were running locally:
...
### Consuming with a Primary Namespace
...
Consume supports all of the above permutations however, in order to use the first one `service-name`, we need to designate a *Primary Namespace*. The Primary Namespace makes the `namespace` optional so you can address the service by its `service-name` as if your local machine is running inside the cluster and within the same namespace.
