...
Consume can support external databases (or any external service) by applying a Kuberentes [service](https://kubernetes.io/docs/concepts/services-networking/service/#externalname) manifest. Apply the following manifests to create an external service using Postgres as an example:
...
:::note
For Codezero to consume external services properly a service manifest must include ports.
:::
