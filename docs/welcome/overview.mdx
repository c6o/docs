...
In the above diagram, a developer is running the Service B Variant locally. This local Service B Variant is _Served_ in the Teamspace and receives traffic from Service A based on _Conditions_ specified by the developer. 
The local Service B Variant then delivers traffic back into the Teamspace by Consuming Service C.
